...
async function swapExactInputSingle(poolAddress, tokenIn, tokenOut, amountIn) {
  ...
  if (allowance.lt(amountIn)) {
    ...
    const approveTx = await tokenInContract.connect(deployer).approve(SWAP_ROUTER_ADDRESS, amountIn);
  ...
  await tokenInContract.connect(deployer).approve(SWAP_ROUTER_ADDRESS, amountIn);
  ...
  // Execute swap
  try {
    const tx = await swapRouter.connect(deployer).exactInputSingle(params, { gasLimit: 500000 });
