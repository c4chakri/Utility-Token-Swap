...
const HeroSection = ({}) => {
  ...
  const callOutPut = async (value) => {
    ...
    const poolAddresses = {
      'ETH-DAI': '0xc2e9f25be6257c210d7adf0d4cd6e3e881ba25f8',
      'ETH-USDC': '0x8ad599c3a0ff1de082011efddc58f1908eb6e6d8',
      // Add more token pairs and their pool addresses
    };
    
      const poolKey = `${tokenOne.symbol}-${tokenTwo.symbol}`;
  const poolAddress = poolAddresses[poolKey] || poolAddresses[`${tokenTwo.symbol}-${tokenOne.symbol}`];

  if (!poolAddress) {
    console.error('Pool not found for the selected token pair');
    return;
  }
